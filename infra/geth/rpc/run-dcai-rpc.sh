...
exec docker run -d --name dcai-rpc --restart unless-stopped \
  -v /opt/dcai/rpc:/data \
  ...
  --syncmode full \
  --http --http.addr 0.0.0.0 --http.port 8545 --http.api eth,net,web3,clique --http.vhosts='*' \
  --ws --ws.addr 0.0.0.0 --ws.port 8546 --ws.api eth,net,web3,clique --ws.origins='*'
